...
export function App() {
  ...
  useEffect(() => { const timer = window.setInterval(() => dispatch({ type: "tick" }), 1000); return () => window.clearInterval(timer); }, []);
  ...
  const active = state.workers.filter(w => w.job !== "idle" && !w.strikeRemaining && isHoused(state, w.id) && state.stock[w.job] < state.warehouseCapacity[w.job]).length;
  ...
  const visible = levels.filter(l => matches(`${l} ${levelInfo[l].title} ${resourceNames[levelInfo[l].job]}`, query) && (filter === "all" || (filter === "open" ? l <= state.level : state.workers.some(w => w.job === levelInfo[l].job && (filter === "strike" ? !!w.strikeRemaining : !w.strikeRemaining && isHoused(state, w.id) && state.stock[levelInfo[l].job] < state.warehouseCapacity[levelInfo[l].job])))));
