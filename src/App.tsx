    ...
    <div className="game-shell">
      <header className="topbar">
        ...
        <nav className="main-navigation" aria-label="Ana menü">
          <button
            aria-current={screen === "production" ? "page" : undefined}
            onClick={() => {
              setScreen("production");
              setCategory(null);
              setSiteId(null);
          ...
          <button
            ...
            aria-label={`Siparişler, ${state.orderPool.length} teklif`}
            onClick={() => setScreen("orders")}
          >
            Siparişler{" "}
            {state.orderPool.length > 0 && (
              <span className="offer-badge">{state.orderPool.length}</span>
            )}
            {state.order && (
              <small>
                Aktif:{" "}
                {state.order.remaining >= 0
                  ? `${state.order.remaining} sn`
                  : "Gecikti"}
              </small>
            )}
          </button>
        ...
        <details className="inventory-overview">
          <summary>
            📦 Envanter · {inventory.length} ürün çeşidi ·{" "}
            {inventory.reduce((sum, r) => sum + state.stock[r], 0)} adet{" "}
      ...
      {screen === "orders" ? (
        ...
        <div className="category-layout">
          <main>
            ...
            {category === "hospital" && (
              <HospitalPanel state={state} act={act} />
            )}
              ...
              <div className="site-detail">
                ...
                <section className="panel equipment-panel">
                  ...
                  <p>
                    ...
                    Kırılmadan yükselt: dayanıklılık %100 olur, ekipman daha
                    uzun dayanır ve tam setin seviyesi üretimi hızlandırır.
                    Kırılan ekipman yok olur.
                  </p>
