...
import { dailyPayroll, dailyWagePerWorker, skipToMorning, isWorkingHours, formatGameTime, togglePause, isHoused, buildShelter, shelterCost, SHELTER_BEDS, warehouseResources, warehouseUpgradeCost, upgradeWarehouse, hireForJob, levels, MAX_LEVEL, canFulfillOrder, fulfillOrder, hiringCost, hireWorker, loadGame, resources, resourceNames, changeWorkers, emptyState, GameState, Resource, levelInfo, simulateTick } from "./game";
...
export function App() {
  ...
  return <div className="game-shell">
    ...
    <div className="dashboard">
      ...
      <section className="panel payroll-card" aria-label="Günlük ücretler"><span className="eyebrow">İŞÇİ GİDERLERİ</span><h2>Günlük ücretler</h2><div className="supply"><span>Kişi başına / gün</span><b>{money(dailyWagePerWorker(state))}</b></div><div className="supply"><span>{state.workers.length} işçi toplam</span><b>{money(dailyPayroll(state))}</b></div><p>Her gün devrinde, güncel yeni işçi alma maliyetinin %1’i tüm işçiler için ödenir. Boşta, grevde ve barınaksız işçiler dahildir. Kasa yetersizse borca düşer.</p></section>
